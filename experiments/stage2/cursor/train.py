...
This is Cursor's implementation of Stage 2 cross-modality alignment
using FluxTalk-style register tokens with nested dropout.

Main training script for Stage 2 cross-modality alignment.
...
from experiments.stage2.cursor.engine import train_one_epoch, evaluate

import wandb


def get_args_parser():
    parser = argparse.ArgumentParser('CURSOR: Stage 2 Cross-Modality Alignment Training', add_help=False)
...
def main(args):
    print("=" * 80)
    print("CURSOR IMPLEMENTATION - FluxTalk Register Tokens")
    print("Experiment ID: cursor_flux_talk_register_tokens")
    print("=" * 80)
    ...
    # Create Stage 2 model - CURSOR IMPLEMENTATION
    print("Creating Cursor's RegisterTokenModule...")
    model = RegisterTokenModule(
    ...
    # Create loss function - CURSOR IMPLEMENTATION
    print("Creating Cursor's RegisterTokenLoss...")
    loss_fn = RegisterTokenLoss(
        active_modalities=modality_types,
        num_overlapping_tokens=args.num_overlapping_tokens,
    ...
    print("CURSOR IMPLEMENTATION - All results will be labeled as 'cursor'")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f"[CURSOR] Acc@1 the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        print(f"[CURSOR] Acc@5 the network on the {len(dataset_val)} test images: {test_stats['acc5']:.1f}%")
        print(f"[CURSOR] Validation Loss: {test_stats['loss']:.4f}")
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                        'epoch': epoch,
                        'implementation': 'cursor'}  # Label as Cursor
    ...
    print('[CURSOR] Training time {}'.format(total_time_str))
